...
import { parsePrefsBody } from "@/lib/parsers";
...
export async function POST(request: Request): Promise<Response> {
  try {
    const body = parsePrefsBody(await readJson(request));
    if (!body) {
      return jsonError("Need eventId.", 400);
    }
    const actor = await actorForBridge(request, body.eventId);
    await lobbyDispatch("updatePrefs", { actor, input: body, eventId: body.eventId });
    return jsonOk(
